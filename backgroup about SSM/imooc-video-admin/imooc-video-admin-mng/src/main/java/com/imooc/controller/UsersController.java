...
//		} else if (username.equals("dzy") && password.equals("123456")) {
//
...
//		}
		...
		if (StringUtils.isBlank(username) || StringUtils.isBlank(password)) {
			return IMoocJSONResult.errorMap("用户名和密码不能为空");
		} else if (rootsService.queryRootForLogin(username,(MD5Utils.getMD5Str(password)))) {

			String token = UUID.randomUUID().toString();
			AdminUser user = new AdminUser(username, (MD5Utils.getMD5Str(password)), token);
			request.getSession().setAttribute("sessionUser", user);
			return IMoocJSONResult.ok();
		}
